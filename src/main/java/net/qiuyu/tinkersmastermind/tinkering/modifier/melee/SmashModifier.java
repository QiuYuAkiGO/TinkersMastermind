...
public class SmashModifier extends SlingModifier implements MeleeHitModifierHook, MeleeDamageModifierHook {
  private final ResourceLocation KEY = new ResourceLocation("tinkersmastermind", "smash"); 
  ...
  public InteractionResult onToolUse(IToolStackView tool, ModifierEntry modifier, Player player, InteractionHand hand, InteractionSource source) {
    if (!tool.isBroken() && source == InteractionSource.RIGHT_CLICK) {
      ...
      tool.getPersistentData().putInt(GeneralInteractionModifierHook.KEY_DRAWTIME, (int)Math.ceil(75f / speed));
        ...
        if (hit != null) {
          ...
          if (mop.getType() != HitResult.Type.BLOCK || targetDist < mop.getBlockPos().distToCenterSqr(start)) {
            ...
            if (tool.hasTag(TinkerTags.Items.MELEE)) {
              ...
              float forcefinal = 0.6124f+(f-0.2f)*0.3876f*1.25f;
              ToolAttackUtil.attackEntity(tool, entity, hand, target, () -> Math.min(1, forcefinal), true);
              isBonking = false;
            }
